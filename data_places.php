...
/////////////////////////////////////////////////////////// Find author names and author codes
	...
	if(($inputReset != "yes")) {
		...
		$query = "SELECT DISTINCT(stated_publication_places) FROM manuscript_books_editions WHERE stated_publication_places LIKE \"%$inputFilter%\" AND stated_publication_places != \"\" AND stated_publication_places IS NOT NULL ORDER BY stated_publication_places ASC";
		...
		if(($found == "y")) {
			...
			if(($c > 0)) {
				foreach($return_arr as $ra) {
					...
					echo "var show = $('#show_results_places').load('./data_places.php?inputReset=yes',function(){});\" ";
