...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////// Sale Form Data	
	...
	$inputSaleReasonText = $_GET["inputSaleReasonText"];			// Sale Form Data
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////// Get Sale ID and Name	
	
	$inputSales = explode("|",$inputSale);
	$inputSaleAgentID = $inputSales[0];
	$inputSaleAgentName = "Not Data-Matched";
	$queryD = "SELECT manuscript_sales_events.Client_Code, manuscript_dealers.Dealer_Name, manuscript_dealers.Profession ";
	$queryD .= "FROM manuscript_sales_events, manuscript_dealers ";
	$queryD .= "WHERE manuscript_sales_events.Client_Code = manuscript_dealers.Client_Code ";
	$queryD .= "AND manuscript_sales_events.salesNumber = \"$inputSaleAgentID\" "; 
	$mysqli_resultD = mysqli_query($mysqli_link, $queryD);
	while($rowD = mysqli_fetch_row($mysqli_resultD)) { 
		$inputSaleAgentName = $rowD[1]." (".$rowD[2].")";
	}
...
/////////////////////////////////////////////////////////// Data debug
	...
	if(($debug == "y")) {
		echo "Datestamp : $dateStamp <br /><br />";
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// Data routines
	...
	if(($inputSuperBookTitle != "") && ($action == "ADD")) {
		$lastID = "";
		$queryD = "INSERT INTO manuscript_events_sales VALUES ( ";
		$queryD .= "0, ";
		$queryD .= "\"$inputArchive\", ";
		$queryD .= "\"\", ";
		$queryD .= "\"\", ";
		$queryD .= "\"\", ";
		$queryD .= "\"$inputPlaceName\", ";
		$queryD .= "\"$inputDealerName\", ";
		$queryD .= "\"$inputSuperBookTitle\", ";
		$queryD .= "\"$inputEditionName\", ";
		$queryD .= "\"$inputEvent\", ";
		$queryD .= "\"$manuscript_copies\", ";
		$queryD .= "\"$manuscript_folio\", ";
		$queryD .= "\"$event_date\", ";
		$queryD .= "\"$citationValue\", ";
		$queryD .= "\"$manuscript_page_stamped\", ";
		$queryD .= "\"$manuscript_notes\", ";
		$queryD .= "\"$manuscript_article\", ";
		$queryD .= "\"$manuscript_othernotes\", ";
		$queryD .= "\"$manuscript_volnotes\", ";
		$queryD .= "\"$event_location\", ";
		$queryD .= "\"$dateStamp\", ";
		$queryD .= "\"$user|$browser\", ";
		$queryD .= "\"$inputSale\", ";
		$queryD .= "\"$inputAgents\", ";
		$queryD .= "\"$inputSaleReason\", ";
		$queryD .= "\"$inputSaleReasonText\", ";
		$queryD .= "\"$inputAgentsRaw\", ";
		$queryD .= "\"$manuscript_morenotes\", ";
		$queryD .= "\"$manuscript_copies_type\", ";
		$queryD .= "\"$manuscript_lotprice\", ";
		$queryD .= "\"$inputSaleAgentID\", ";
		$queryD .= "\"$inputSaleAgentName\" ";
		$queryD .= ")";
		$mysqli_resultD = mysqli_query($mysqli_link, $queryD);	
	...
	if(($action == "DELETE") && ($ID != "")) {
		$queryD = "DELETE FROM manuscript_events_sales WHERE ID = \"$ID\" ";
		$mysqli_resultD = mysqli_query($mysqli_link, $queryD);
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// Pagination
	...
	$query_pag_num = "SELECT COUNT(*) AS count FROM manuscript_events_sales";
...
?>
						<div class="page-header">
                			<h3><?php echo $alias; echo " (Sales)"; if(($searchText)) { echo " : ".$searchText; } ?></h3>
              			</div>
                        <p>
...
///////////////////////////////// FOR ENABLING THE FIRST BUTTON
	...
	if ($first_btn && $cur_page > 1) {
		...
		echo "var dataE = 'alias=".$alias."&filter=".$filter."&page=1'; ";
		echo "var enameVal = $('#manuscriptRecords').load('./data_records_sales.php',dataE);\">";
		echo "First</a></li>";
	} else if ($first_btn) {
...
///////////////////////////////// FOR ENABLING THE PREVIOUS BUTTON
	...
	if ($previous_btn && $cur_page > 1) {
		...
		echo "<li>";
		echo "<a href=\"javascript: ";
		echo "var dataE = 'alias=".$alias."&filter=".$filter."&page=".$pre."'; ";
		echo "var enameVal = $('#manuscriptRecords').load('./data_records_sales.php',dataE);\">";
		echo "&laquo;</a></li>";
...
///////////////////////////////// LOPP THROUGH PAGES
	...
	for ($i = $start_loop; $i <= $end_loop; $i++) {
		if ($cur_page == $i) {
			...
			echo "<a href=\"javascript: ";
			echo "var dataE = 'alias=".$alias."&filter=".$filter."&page={$i}&searchText=".$searchText."&action=".$action."'; ";
			echo "var enameVal = $('#manuscriptRecords').load('./data_records_sales.php',dataE);\">";
			echo "{$i}</a></li>";
    	} else {
        	echo "<li>";
...
/////////////////////////////////////////////////////////// Start data loop
						...
						$queryD = "SELECT * FROM manuscript_events_sales ";
						...
						while($rowD = mysqli_fetch_row($mysqli_resultD)) {
							...
							if(($rowD[9] == "Stock Sale")) { $event = "<strong><span style=\"color: #258fb2;\">S</span></strong>"; }
							if(($rowD[9] == "Sale of Privilege")) { $event = "<strong><span style=\"color: #28af26;\">P</span></strong>"; }
...
/////////////////////////////////////////// Main record							
							...
							echo "<td $styleColor width=\"25%\"><p style=\"text-align:left;\">$rowD[31]</p></td>";
							echo "<td $styleColor >$rowD[25]</td>";
							...
							echo "if (r == true) { var doThis = $('#manuscriptRecords').load('./data_records_sales.php',dataE); }\">";
...
/////////////////////////////////////////// Supplementary record data	
							...
							echo "<span style=\"color: #9b5972;\">Sales Record ID $rowD[0]</span>";
							echo "<br />$dateEntered";
							echo "<br />";
							if(($eFolio != "")) { 
								echo "Folio $eFolio "; 
